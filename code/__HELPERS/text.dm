...
//RUS//
/proc/sanitize_rus(var/input, var/max_length = MAX_MESSAGE_LEN, var/encode = 1, var/trim = 1, var/extra = 1, var/ja_mode = CHAT)
	if(!input)
		return
	if(max_length)
		input = copytext(input,1,max_length)
...
/proc/sanitizeSafe_rus(var/input, var/max_length = MAX_MESSAGE_LEN, var/encode = 1, var/trim = 1, var/extra = 1, var/ja_mode = CHAT)
	return sanitize_rus(replace_characters(input, list(">"=" ","<"=" ", "\""="'")), max_length, encode, trim, extra, ja_mode)

/proc/sanitize_rus_uni(var/input, var/max_length = MAX_MESSAGE_LEN, var/encode = 1, var/trim = 1, var/extra = 1, var/ja_mode = POPUP)
	if(!input)
		return
	if(max_length)
		input = copytext(input,1,max_length)
...
/proc/sanitizeSafe_uni(var/input, var/max_length = MAX_MESSAGE_LEN, var/encode = 1, var/trim = 1, var/extra = 1, var/ja_mode = POPUP)
	return sanitize_rus(replace_characters(input, list(">"=" ","<"=" ", "\""="'")), max_length, encode, trim, extra, ja_mode)
